...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      {
        // if two or one live neighbors, keep alive; otherwise die.
        if(neighbors==2 || neighbors==1) result = 1; else result = 0;
      }
     ...
     // write pixels into destination image, scaled to 0 or 255:
      ...
      if(result == 1){
        ...
        noStroke();
        rect(i/howwide*width, j/howtall*height, 5, 5);
      }
    }
...
{
  fillatmouse();
 // background(random(255), random(255), randon(255), 40);
}
